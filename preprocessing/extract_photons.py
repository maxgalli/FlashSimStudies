        ...
        other = ak.Array(
            {
                ...
                "GenPhoGenEle_deltaeta": abs(nearest_gen_photons.eta - nearest_gen_electrons.eta),
                ...
                "GenPhoGenJet_deltaeta": abs(nearest_gen_photons.eta - nearest_gen_jets.eta),
...
def main(args):
    base_dir = "/pnfs/psi.ch/cms/trivcat/store/user/gallim/FlashSim/GJet"
    files = [os.path.join(base_dir, f) for f in os.listdir(base_dir) if f.endswith(".root")]
    #files = files[:1]
    fileset = {"GJet": files}
    print(fileset)
    output_dir = "/work/gallim/SIMStudies/FlashSimStudies/preprocessing/extracted_photons"
